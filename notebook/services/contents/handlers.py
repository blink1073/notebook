...
class ContentsHandler(APIHandler):
    ...
    def _save(self, model, path):
        ...
        chunk = model.get("chunk", None) # CHANGE_FLAG
        if not chunk or chunk == -1:  # CHANGE_FLAG: to avoid tedious log information
            self.log.info(u"Saving file at %s", path)  # CHANGE_FLAG
        #  self.log.info(u"Saving file at %s", path)  # CHANGE_FLAG
